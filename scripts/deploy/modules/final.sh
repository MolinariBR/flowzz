...
if [ "$DOMAIN" = "localhost" ]; then
    ...
    echo -e "  🏠 Landing Page:   https://$DOMAIN (Primeira página)"
    echo -e "  👤 Dashboard:      https://app.$DOMAIN (Área do usuário)"
...
echo -e "  $DOMAIN           → IP_DO_SERVIDOR (Landing Page)"
echo -e "  www.$DOMAIN       → IP_DO_SERVIDOR"
echo -e "  app.$DOMAIN       → IP_DO_SERVIDOR (Dashboard do usuário)"
echo -e "  admin.$DOMAIN     → IP_DO_SERVIDOR (Admin)"
echo -e "  api.$DOMAIN       → IP_DO_SERVIDOR (API)"