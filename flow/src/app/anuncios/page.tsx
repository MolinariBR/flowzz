...
export default function Anuncios() {
  ...
  const campaigns = insightsData?.campaigns
    ? insightsData.campaigns.map((campaign, _index) => {
        // Encontrar insights correspondentes para esta campanha
        const campaignInsights = insightsData.insights.filter(
        ...
        const totalImpressions = campaignInsights.reduce(
          (sum, insight) => sum + insight.impressions,
          0
        )
        const totalClicks = campaignInsights.reduce((sum, insight) => sum + insight.clicks, 0)
        const avgCTR =
          campaignInsights.length > 0
            ? campaignInsights.reduce((sum, insight) => sum + insight.ctr, 0) /
              campaignInsights.length
            : 0
        const avgROAS =
          campaignInsights.length > 0
            ? campaignInsights.reduce((sum, insight) => sum + (insight.roas || 0), 0) /
              campaignInsights.length
            : 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Facebook Ads Manager</h1>
          <p className="text-slate-600 mt-1">Gerencie suas campanhas como um profissional</p>
        </div>
      ...
      {integrationStatus && (
        <div
          className={`flex items-center space-x-2 p-4 rounded-lg ${integrationStatus.connected ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}
        >
          {integrationStatus.connected ? (
            <>
              <div className="w-2 h-2 bg-green-500 rounded-full"></div>
              <span className="text-sm text-green-800">
                Conectado ao Facebook Ads • {integrationStatus.adAccounts?.length || 0} conta(s) de
                anúncio
                {integrationStatus.lastSync &&
                  ` • Última sincronização: ${new Date(integrationStatus.lastSync).toLocaleString('pt-BR')}`}
              </span>
            </>
          ) : (
            <>
              ...
              <span className="text-sm text-yellow-800">
                Não conectado ao Facebook Ads. Clique em &quot;Conectar Facebook Ads&quot; para
                começar.
              </span>
            </>
          )}
      ...
      {integrationStatus?.connected && insightsData ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          <MetricCard
            ...
            icon={DollarSign}
          />
          <MetricCard title="Impressões" value="117.300" change="+8,2%" trend="up" icon={Eye} />
          <MetricCard
            title="Cliques"
            value="3.355"
            change="+15,7%"
            ...
            icon={MousePointer}
          />
          <MetricCard title="CPM" value="R$ 7,04" change="-5,3%" trend="down" icon={Target} />
          <MetricCard title="CPC" value="R$ 0,25" change="-8,1%" trend="down" icon={Target} />
          <MetricCard title="CTR" value="2,86%" change="+3,2%" trend="up" icon={TrendingUp} />
          <MetricCard title="ROAS" value="9,7x" change="+18,5%" trend="up" icon={TrendingUp} />
        </div>
      )}
      ...
      {integrationStatus?.connected && insightsData ? (
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Performance das Campanhas</h3>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Performance das Campanhas</h3>
              <p className="text-sm text-slate-600">Gasto vs Cliques - Últimos 7 dias</p>
            </div>
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 bg-blue-500 rounded-full"></div>
              ...
              {campaigns.map((campaign) => (
                <>
                  ...
                  <tr className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-mono font-semibold">R$ {campaign.spent}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`font-mono font-semibold ${getRoasColor(campaign.roas)}`}>
                        {campaign.roas}x
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <button type="button" className="p-1 text-red-600 hover:bg-red-50 rounded">
                  ...
                  {expandedCampaigns.includes(campaign.id) &&
                    campaign.adSets.map((adSet) => (
                      <>
                        <tr className="bg-slate-25 hover:bg-slate-50">
                          ...
                          <td className="px-6 py-3">
                            <div className="flex items-center space-x-2 ml-6">
                              ...
                              <div>
                                <p className="font-medium text-slate-800 text-sm">{adSet.name}</p>
                          ...
                          <td className="px-6 py-3">
                            <span className="font-mono text-sm">R$ {adSet.spent}</span>
                          </td>
                          <td className="px-6 py-3">
                          ...
                          <td className="px-6 py-3">
                            <span className="font-mono text-sm">{adSet.clicks}</span>
                          </td>
                          <td className="px-6 py-3">
                            <span className="font-mono text-sm">{adSet.ctr}%</span>
                          </td>
                          <td className="px-6 py-3">
                            <span
                        ...
                        {/* Ads */}
                        {adSet.ads.map((ad) => (
                          <tr key={ad.id} className="bg-slate-25 hover:bg-slate-50">
                            <td className="px-6 py-2"></td>
                            <td className="px-6 py-2">
                              <div className="flex items-center space-x-2 ml-12">
                                <div className="w-4 h-px bg-slate-300"></div>
                                <div>
                                  <p className="font-medium text-slate-700 text-sm">{ad.name}</p>
                                  <p className="text-xs text-slate-400">Anúncio</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-2">
                            ...
                            <td className="px-6 py-2">
                              <span className="font-mono text-sm">R$ {ad.spent}</span>
                            </td>
                            <td className="px-6 py-2">-</td>
                            <td className="px-6 py-2">-</td>
                            <td className="px-6 py-2">-</td>
