...
export default function Projecoes() {
	...
	const { projections, goals, isLoading, fetchProjections, createGoal, updateGoal, deleteGoal } = useProjections();
	...
	const aggregatedMetrics = projections.reduce(
		(acc, projection) => ({
			...
			totalProfit: acc.totalProfit + ((projection.projectedValue || 0) - (projection.currentValue || 0)),
		}),
		{ totalRevenue: 0, totalExpenses: 0, totalProfit: 0 }
	);

	const profitMargin = aggregatedMetrics.totalRevenue > 0
					...
					>
						...
						{isLoading ? (
							...
							<div className="space-y-4">
								{projections.filter(p => p.type === "revenue").map((projection) => (
									<div key={projection.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
										<div>
							...
							<div key={projection.id} className="p-4 bg-gray-50 rounded-lg">
								...
								<div className="space-y-2 text-sm">
									<div className="flex justify-between">
										...
										<span className="font-medium">{formatCurrency(projection.scenarios.pessimistic)}</span>
									</div>
									<div className="flex justify-between">
										<span className="text-gray-600">Realista:</span>
										<span className="font-medium">{formatCurrency(projection.scenarios.realistic)}</span>
									</div>
									<div className="flex justify-between">
										<span className="text-gray-600">Otimista:</span>
										<span className="font-medium">{formatCurrency(projection.scenarios.optimistic)}</span>
									</div>
								</div>
							</div>
						))}
