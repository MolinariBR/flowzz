...
export const useClients = (isAuthenticated: boolean = false) => {
  ...
  const loadClients = useCallback(async (clientFilters?: ClientFilters) => {
  ...
  // Criar novo cliente
  const createNewClient = useCallback(async (data: CreateClientInput) => {
    try {
      setIsLoading(true)
  ...
  // Atualizar cliente
  const updateExistingClient = useCallback(async (id: string, data: UpdateClientInput) => {
    try {
      setIsLoading(true)
  ...
  const addTagToExistingClient = useCallback(async (clientId: string, tagId: string) => {
    try {
