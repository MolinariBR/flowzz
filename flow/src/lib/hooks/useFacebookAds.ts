...
export const useFacebookAds = () => {
  ...
  // Buscar insights
  const getInsights = useCallback(async (params: FacebookInsightsParams): Promise<FacebookInsightsResponse | null> => {
    try {
      setIsLoading(true);
  ...
  const handleCallback = useCallback(async (code: string, state: string): Promise<FacebookCallbackResponse | null> => {
    try {
      setIsLoading(true);
