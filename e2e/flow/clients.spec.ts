...
test.describe('Flow - Clients Management', () => {
  ...
  test('deve listar clientes existentes', async ({ page }) => {
    ...
    const emptyState = await page
      .getByText(/nenhum cliente|adicione seu primeiro cliente/i)
      .isVisible()
  ...
  test('deve abrir modal de criar cliente', async ({ page }) => {
    ...
    const newClientButton = page.getByRole('button', {
      name: /novo cliente|adicionar cliente|criar cliente/i,
    })
  ...
  test('deve abrir detalhes do cliente ao clicar na linha', async ({ page }) => {
    ...
    if (await firstClient.isVisible()) {
      ...
      await expect(page.getByText(/detalhes do cliente|informações/i)).toBeVisible({
        timeout: 3000,
      })
    }
  })
