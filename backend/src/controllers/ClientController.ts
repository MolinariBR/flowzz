...
export class ClientController {
  ...
  getClients = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const result = await this.clientService.getClients(req.user.userId, page, limit, cleanFilters)
  ...
  createClient = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('Já existe um cliente') || error.message.includes('inválido')) {
