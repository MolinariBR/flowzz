...
describe('Authentication E2E Tests', () => {
  ...
  const createTestUser = () => ({
    email: `test-existing-${Date.now()}@flowzz.com`,
    password: 'Test@123456',
    nome: 'Test Existing User',
    role: 'USER' as const,
  })
  ...
  afterAll(async () => {
    ...
    await prisma.refreshToken.deleteMany({
      where: { user: { email: { contains: 'test-existing-' } } },
    })
    await prisma.user.deleteMany({
      where: { email: { contains: 'test-existing-' } },
    })
  })
  ...
  describe('POST /api/v1/auth/register', () => {
    it('deve registrar um novo usuário com trial de 7 dias', async () => {
      ...
      _accessToken = response.body.data.tokens.accessToken
      _refreshToken = response.body.data.tokens.refreshToken
    ...
    it('deve retornar 400 se email já existir', async () => {
      ...
      const password_hash = await bcrypt.hash(existingUser.password, 10)
