...
describe('FacebookAdsService', () => {
  ...
  describe('OAuth 2.0 Flow', () => {
    ...
    describe('handleOAuthCallback', () => {
      ...
      it('deve rejeitar state expirado (>15 minutos)', async () => {
        ...
        const expiredState = Buffer.from(`${userId}:${oldTimestamp}:randomstring`).toString(
          'base64'
        )
  ...
  describe('Insights & Sync', () => {
    describe('getAdAccountInsights', () => {
      ...
      it('deve buscar insights da API quando cache miss', async () => {
        ...
        mockAxios.get.mockResolvedValueOnce({
          data: {
            data: [{ id: 'campaign_1', name: 'Campaign 1', status: 'ACTIVE' }],
          },
        })
      ...
      it('deve respeitar rate limit (200 req/hora)', async () => {
        ...
        await expect(facebookAdsService.getAdAccountInsights(userId, params)).rejects.toThrow(
          'rate limit'
        )
      })
    })
  ...
  describe('Error Handling', () => {
    it('deve tratar erro de rede ao buscar insights', async () => {
      ...
      ;(prisma.integration.findUnique as any).mockRejectedValue(new Error('Network error'))
    ...
    it('deve tratar erro de integração não encontrada', async () => {
      ...
      await expect(facebookAdsService.getAdAccounts(userId)).rejects.toThrow('not connected')
    })
    ...
    it('deve tratar erro de token inválido', async () => {
      ...
      await expect(facebookAdsService.getAdAccounts(userId)).rejects.toThrow()
    })
  })
})
