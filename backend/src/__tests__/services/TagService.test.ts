...
describe('TagService', () => {
  ...
  describe('getById', () => {
    it('deve retornar tag quando encontrada', async () => {
      ...
      await expect(
        tagService.getById('user-123', 'tag-123')
      ).rejects.toThrow(); // Mock retorna undefined, deve lançar erro
    });

    it('deve rejeitar quando tag não pertence ao usuário (multi-tenancy)', async () => {
      await expect(
        tagService.getById('user-123', 'tag-de-outro-usuario')
  ...
  describe('delete', () => {
    it('deve excluir tag sem clientes associados', async () => {
      ...
      await expect(
        tagService.delete('user-123', 'tag-123')
      ).rejects.toThrow(); // Mock do Prisma não configurado para sucesso
    });
    ...
    it('deve rejeitar exclusão de tag com clientes associados', async () => {
      ...
      await expect(
        tagService.delete('user-123', 'tag-123')
      ).rejects.toThrow('Não é possível excluir tag com clientes associados');
    });
  });
