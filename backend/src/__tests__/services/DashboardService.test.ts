...
import type { DashboardMetrics } from '../../interfaces/DashboardService.interface';
...
describe('DashboardService', () => {
  ...
  describe('getMetrics', () => {
    ...
    it('deve calcular métricas quando não há cache', async () => {
      ...
      mockRepository.getSalesForPeriod = vi.fn()
      ...
      mockRepository.getAdSpendForPeriod = vi.fn()
      ...
      mockRepository.getScheduledPayments = vi.fn()
        .mockResolvedValue([{ total_price: 1500, status: 'PENDING' }, { total_price: 500, status: 'SHIPPED' }]);
