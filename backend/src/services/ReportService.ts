...
export class ReportService implements IReportService {
  ...
  async collectSalesData(userId: string, filters: ReportFilters): Promise<SalesReportData> {
    ...
    logger.info('Cache miss for sales data, fetching from database', {
      userId,
      cacheKey,
    })
    const where: {
    ...
    const conversionRate =
      totalSales > 0
        ? (sales.filter((s) => s.status === 'PAID' || s.status === 'DELIVERED').length /
            totalSales) *
          100
        : 0
  ...
  async collectFinancialData(userId: string, filters: ReportFilters): Promise<FinancialReportData> {
    ...
    logger.info('Cache miss for financial data, fetching from database', {
      userId,
      cacheKey,
    })
  ...
  async collectClientsData(userId: string, filters: ReportFilters): Promise<ClientsReportData> {
    ...
    const topClients = clients
      .map((client) => ({
        ...
        lastPurchase:
          client.sales.length > 0 && client.sales[0]
            ? client.sales.reduce(
                (latest, sale) => (sale.created_at > latest ? sale.created_at : latest),
                client.sales[0].created_at
              )
            : client.created_at,
      }))
  ...
  async getStatistics(userId: string): Promise<ReportStatistics> {
    ...
    const reportsByFormat = [{ format: ReportFormat.PDF, count: totalReports }]
    ...
    const lastGenerated =
      reports.length > 0 && reports[0]
        ? reports.reduce(
            (latest, report) => (report.created_at > latest ? report.created_at : latest),
  ...
  private groupSalesByDay(
    sales: Array<{
      created_at: Date
      total_price: number | { toNumber: () => number }
    }>
  ...
  private groupByMonth(
    ...
    ads: Array<{ date: Date; spend: number | { toNumber: () => number } }>
  ) {
