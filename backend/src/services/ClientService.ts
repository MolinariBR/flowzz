...
export class ClientService {
  ...
  async getClients(
    ...
    const result = await this.clientRepository.findByUserId(userId, filters, {
      page,
      limit,
    })
  ...
  private async validateClientData(
    data: Omit<CreateClientDTO, 'user_id'>,
    userId: string
  ): Promise<void> {
  ...
  private async validateEmailUniqueness(
    ...
    const existing = await this.clientRepository.findByUserId(userId, filters, {
      page: 1,
      limit: 1,
    })

    const duplicateClient = existing.data.find(
      (client) => client.email === email && client.id !== excludeId
    )
