...
export class ExcelGenerator {
   ...
   */
  async generateSalesReportExcel(data: SalesReportData, options: ReportOptions): Promise<Buffer> {
    try {
      ...
      const clientesData = [
        ...
        ...data.topClients.slice(0, 10).map(
          (
            client: {
              id: string
              name: string
            ...
            index: number
          ) => [index + 1, client.name, client.orderCount, this.formatCurrency(client.totalSpent)]
        ),
      ]
      ...
      throw new Error(
        `Falha ao gerar relatório Excel: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      )
    }
  }
    ...
    try {
      ...
      const receitasData = [
        ...
        ...data.revenueByMonth.map(
          (item: { month: string; revenue: number; expenses: number; profit: number }) => [
            item.month,
            this.formatCurrency(item.revenue),
   ...
   */
  async generateAdsReportExcel(data: AdsReportData, options: ReportOptions): Promise<Buffer> {
    try {
  ...
  async generateClientsReportExcel(
    ...
    try {
      ...
      const topClientesData = [
        ...
        ...data.topClients.slice(0, 20).map(
          (
            client: {
              id: string
              name: string
